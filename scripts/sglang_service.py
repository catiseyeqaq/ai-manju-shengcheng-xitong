...
Runtime weights must live under /root/models (local disk). /workdata is backup only.
"""
...
MODEL_PATH = Path(
    os.environ.get("SGLANG_POLISH_MODEL_PATH", "/root/models/Qwen3.6-35B-A3B")
).resolve()
...
LAUNCHER = Path(
    os.environ.get(
        ...
        "/workdata/GNN+LLM/services/sglang_launch_ppu.py",
    )
)
...
def build_cmd() -> list[str]:
    ...
    if not MODEL_PATH.is_dir():
        raise FileNotFoundError(
            ...
            "Runtime weights must be under /root/models (see MODEL_LOCATIONS_20260810.md)."
        )
    return [
        str(PYTHON_BIN),
        str(LAUNCHER),
