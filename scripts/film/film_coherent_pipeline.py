...
def copy_output_to_input(filename: str, dest_name: str) -> str:
    ...
    matches = sorted(Path("ComfyUI/output").rglob(filename.replace("%", "*")))
    # Also search by prefix
    prefix = dest_name
    cands = sorted(Path("ComfyUI/output").rglob(f"{prefix}*.png"))
    if not cands:
        cands = sorted(Path("ComfyUI/output/film_coherent").rglob(f"*{prefix}*.png"))
    if not cands:
...
def main() -> int:
    ...
    pulid_weight = Path("ComfyUI/models/pulid/pulid_flux_v0.9.1.safetensors")
