#!/bin/bash
LOG=ComfyUI/logs/asr_monitor.log
OUT=ComfyUI/output/after_school_road
...
while true; do
  {
    ...
    # latest log lines
    echo "  -- bible log --"
    tail -3 ComfyUI/logs/asr_bible.log 2>/dev/null | sed 's/^/  /'
    echo "  -- plates log --"
    tail -2 ComfyUI/logs/asr_plates.log 2>/dev/null | sed 's/^/  /'
    echo "  -- supervisor --"
    tail -3 ComfyUI/logs/asr_supervisor.log 2>/dev/null | sed 's/^/  /'
  } | tee -a "$LOG"
