...
| 存储策略 | 运行权重在本地盘 `/root/models`；`/workdata`（ossfs）仅作持久备份 |
...
- 运行路径：`/root/models/MiniMax-H3-ComfyUI`
- 备份路径：`/workdata/ComfyUI/models_backup/MiniMax-H3-ComfyUI`
...
| 项目 | 值 |
|---|---|
| 路径 | `/root/models/Qwen3.6-35B-A3B` |
| 大小 | ≈ 67G（全量 BF16） |
...
# /root/models/MiniMax-H3-ComfyUI/{diffusion_models,text_encoders,vae}/
...
| 变量 | 默认 | 含义 |
|---|---|---|
| `COMFYUI_ROOT` | `/root/ComfyUI` | ComfyUI 根目录 |
| `COMFYUI_PORT` | `8188` | WebUI 端口 |
| `COMFYUI_H3_MODEL_SRC` | `/root/models/MiniMax-H3-ComfyUI` | H3 权重 |
